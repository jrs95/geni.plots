...
* `geni_test_region`- a `list` containing:
  ...
  * `corr` - a `matrix` of correlation statsitics for the markers in `assoc` using the European samples from [1000 Genomes Phase 3](https://www.internationalgenome.org/). The genomic coordinates used are GRCh37.  
* `geni_test_stack_region`- a `list` containing:
  * `assoc` - a `data.frame` of results from GWAS studies on interleukin-6 and interleukin 6 recepter levels for the GRCh37 region 1:154301970-154551970 ([GCST90274815](https://www.ebi.ac.uk/gwas/studies/GCST90274815) and [GCST90088597](https://www.ebi.ac.uk/gwas/studies/GCST90088597)). The genomic coordinates used are GRCh37. A description of this data can be viewed using `help(geni_test_region)`.  
  * `corr` - a `matrix` of correlation statsitics for the markers in `assoc` using the European samples from [1000 Genomes Phase 3](https://www.internationalgenome.org/). The genomic coordinates used are GRCh37.  
...
```{r phewas, echo = TRUE, eval = TRUE, out.width = "100%"}
fig_phewas(
...
```{r regional, echo = TRUE, eval = TRUE, out.width = "100%"}
fig_region(
...
```{r regional-stack, echo = TRUE, eval = TRUE, out.width = "100%"}
fig_region_stack(
